"""
Symbolic-only predictor (Layer 3): LSTM over key-invariant chord tokens -> (valence, arousal).
...
Outputs (run directory):
  ...
  experiments/symbolic_lstm/run_###/preds.csv
  experiments/symbolic_lstm/run_###/model.pt
"""
...
LOG = logging.getLogger("train_symbolic_lstm")
...
class SymbolicDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int, torch.Tensor, int]:
        r = self.df.iloc[idx]
        sid = int(r["song_id"])
        ...
        x = torch.tensor(ids, dtype=torch.long)  # Shape: [T]
        return x, length, y, sid


def collate_batch(batch: List[Tuple[torch.Tensor, int, torch.Tensor, int]], pad_id: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    xs, lens, ys, sids = zip(*batch)

    lens_t = torch.tensor(lens, dtype=torch.long)  # Shape: [B]
    sids_t = torch.tensor(sids, dtype=torch.long)  # Shape: [B]
    y_t = torch.stack(list(ys), dim=0)             # Shape: [B, 2]

    max_len = int(max(lens))
    x_pad = torch.full((len(xs), max_len), pad_id, dtype=torch.long)  # Shape: [B, Tmax]
    for i, x in enumerate(xs):
        x_pad[i, : x.shape[0]] = x

    return x_pad, lens_t, y_t, sids_t


class SymbolicLSTMRegressor(nn.Module):
    def __init__(self, vocab_size: int, emb_dim: int, hidden_dim: int, n_layers: int, dropout: float) -> None:
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True,
            bidirectional=False,
        )  # Packed -> h_n: [n_layers, B, H]
        self.head = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),  # Shape: [B, H] -> [B, H]
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, 2),           # Shape: [B, H] -> [B, 2]
        )
    ...
    def forward(self, x_ids: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        _, (h_n, _) = self.lstm(packed)  # h_n Shape: [L, B, H]
        h_last = h_n[-1]                 # Shape: [B, H]
        out = self.head(h_last)          # Shape: [B, 2]
        return out
...
def eval_loader(model: nn.Module, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    ...
    for x_ids, lengths, y, _sids in loader:
        x_ids = x_ids.to(device)
        lengths = lengths.to(device)
        y = y.to(device)

        yhat = model(x_ids, lengths)  # Shape: [B, 2]
        ys.append(y)
        yh.append(yhat)
...
def main() -> None:
    ...
    p.add_argument("--exp_root", type=str, default=str(REPO_ROOT / "experiments" / "symbolic_lstm"))
    ...
    req = {"song_id", "split", "valence", "arousal"}
    ...
    n_empty = sum(1 for sid in df["song_id"].astype(int).tolist() if len(token_map.get(sid, [])) < args.min_len)
    LOG.info("Songs total=%d empty_or_short=%d", len(df), n_empty)
    ...
    ds_train = SymbolicDataset(df_train, token_map, vocab, max_len=args.max_len, min_len=args.min_len)
    ds_val = SymbolicDataset(df_val, token_map, vocab, max_len=args.max_len, min_len=args.min_len)
    ds_test = SymbolicDataset(df_test, token_map, vocab, max_len=args.max_len, min_len=args.min_len)
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        for step, (x_ids, lengths, y, _sids) in enumerate(dl_train, start=1):
            x_ids = x_ids.to(device)
            lengths = lengths.to(device)
            y = y.to(device)

            if epoch == 1 and step == 1:
                pad_frac = float((x_ids == vocab.pad_id).float().mean().item())
                LOG.info("First batch: pad_frac=%.4f len_mean=%.2f len_std=%.2f",
                         pad_frac, float(lengths.float().mean().item()), float(lengths.float().std(unbiased=False).item()))
    ...
    rows = []
    with torch.no_grad():
        for x_ids, lengths, y, sids in dl_test:
            x_ids = x_ids.to(device)
            lengths = lengths.to(device)
            yhat = model(x_ids, lengths).detach().cpu().numpy()
